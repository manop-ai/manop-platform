'use client'
// app/agency/dashboard/page.tsx — Agency Dashboard with Cloudinary
// Replaces Supabase Storage with Cloudinary for image uploads
// Everything else stays the same from agency-dashboard.tsx
...
interface Partner {
  ...
  trust_level: string; cities: string[]; contact_email: string | null
}
interface Listing {
...
// ─── Add Listing Form with Cloudinary ─────────────────────────
function AddListingForm({ partnerId, agencyName, dark, onSaved }: {
  ...
  const INP: React.CSSProperties = {
    ...
    fontSize: '0.875rem', padding: '0.6rem 0.875rem',
    outline: 'none', fontFamily: 'inherit',
  }
  const LBL: React.CSSProperties = {
    fontSize: '0.68rem', color: text2, marginBottom: '0.3rem',
  ...
  async function handleSave() {
    ...
    try {
      let ngnRate = 1570
      try {
        // Add 5s timeout for exchange rate API — common issue in Nigeria
        ...
        // Use fallback rate if API times out or fails
        console.warn('Exchange rate API unavailable, using fallback:', ngnRate)
      }

      const { error: dbErr } = await sb.from('properties').insert({
        data_partner_id:     partnerId,
        source_type:         'agent-direct',
        country_code:        neighborhood.includes('Abuja') ? 'NG'
                           : neighborhood.includes('Accra') ? 'GH'
                           : neighborhood.includes('Nairobi') ? 'KE' : 'NG',
        city:                neighborhood.includes('Abuja') ? 'Abuja'
                           : neighborhood.includes('Accra') ? 'Accra'
                           : neighborhood.includes('Nairobi') ? 'Nairobi' : 'Lagos',
        neighborhood,
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        ...
        <div>
          ...
          {priceM && parseMillion(priceM) && (
            <div style={{ fontSize: '0.62rem', color: text3, marginTop: 3 }}>
              = {fmtNGN(parseMillion(priceM))}
            </div>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <input style={INP} type="tel" value={agentPhone}
            onChange={e => setAgentPhone(e.target.value)} placeholder="+234 800 000 0000" />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          <textarea
            ...
            placeholder="Key features, amenities, nearby landmarks..." />
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <ImageUploader
          ...
          hint="JPG, PNG, WebP — up to 10MB each · Auto-optimized via Cloudinary CDN · First photo is cover"
        />
        {imageUrls.length > 0 && (
          <div style={{ fontSize: 11, color: '#22C55E', marginTop: 6 }}>
            ✓ {imageUrls.length} photo{imageUrls.length > 1 ? 's' : ''} ready — will be saved with listing
          </div>
        )}
      </div>
      ...
      <button
        onClick={handleSave}
        disabled={saving || imageUrls.some(u => !u)}
        style={{
          width: '100%', background: '#5B2EFF', color: '#fff', border: 'none',
          borderRadius: 10, padding: '0.875rem', fontSize: '0.9rem', fontWeight: 700,
...
export default function AgencyDashboard() {
  ...
  const INP: React.CSSProperties = {
    ...
    padding: '0.65rem 0.875rem', fontFamily: 'inherit', width: '100%',
  }
  ...
  async function handleLogin() {
    ...
    try {
      ...
      const r1 = await sb.from('data_partners')
        .select('id,name,partner_type,trust_level,cities,contact_email')
        .eq('contact_email', loginVal.trim().toLowerCase()).eq('active', true).single()
      if (r1.data) data = r1.data as Partner

      if (!data && loginVal.includes('-') && loginVal.length > 30) {
        const r2 = await sb.from('data_partners')
          .select('id,name,partner_type,trust_level,cities,contact_email')
          .eq('id', loginVal.trim()).eq('active', true).single()
        if (r2.data) data = r2.data as Partner
      }

      if (!data) {
        const r3 = await sb.from('data_partners')
          .select('id,name,partner_type,trust_level,cities,contact_email')
          .ilike('name', `%${loginVal.trim()}%`).eq('active', true).limit(1).single()
        if (r3.data) data = r3.data as Partner
      }

      if (!data) {
        setLoginErr('No agency found. Check the email or name, or register at /agency/onboard')
        setLogging(false); return
      }
      setPartner(data)
      loadListings(data.id)
    } catch {
      setLoginErr('Login failed. Try again or email partners@manopintel.com')
  ...
  async function saveEdit(id: string) {
    ...
    if (!price) { setSaveMsg('Enter a valid price in millions'); return }
    let ngnRate = 1570
    try {
      const r = await fetch('https://open.er-api.com/v6/latest/USD')
      const d = await r.json()
    ...
    await sb.from('properties').update({
      price_local: price, price_usd: Math.round(price / ngnRate)
    }).eq('id', id)
    setEditId(null)
      ...
      <div style={{ maxWidth: 420, width: '100%' }}>
        ...
        <div style={{ background: bg3, border: `1px solid ${border}`, borderRadius: 16, padding: '2rem' }}>
          ...
          <p style={{ fontSize: 13, color: text2, marginBottom: '1.5rem', lineHeight: 1.55 }}>
            Enter your agency email, partner ID, or agency name to access your dashboard.
          </p>
          <label style={{ fontSize: 12, color: text2, display: 'block', marginBottom: 5, fontWeight: 500 }}>Email / Partner ID / Agency name</label>
          <input
            style={{ ...INP, marginBottom: 10 }}
            placeholder="e.g. CW Real Estate or listings@cw.ng"
            value={loginVal}
          ...
          />
          {loginErr && (
            <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 8, padding: '0.6rem 0.875rem', fontSize: 13, color: '#EF4444', marginBottom: 10 }}>
              {loginErr}
            </div>
          )}
          <button
            onClick={handleLogin} disabled={logging}
            style={{ width: '100%', background: '#5B2EFF', color: '#fff', border: 'none', borderRadius: 8, padding: '0.75rem', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: logging ? 0.7 : 1 }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem' }}>
        ...
        {tab === 'overview' && (
          <>
            ...
            {listings.length > 5 && (
              <button onClick={() => setTab('listings')} style={{ fontSize: 13, color: '#7C5FFF', background: 'transparent', border: 'none', cursor: 'pointer', padding: '0.5rem 0' }}>
                ...
                return (
                  <div key={l.id} style={{ background: bg3, border: `1px solid ${isEditing ? 'rgba(91,46,255,0.4)' : border}`, borderRadius: 10, marginBottom: 8, overflow: 'hidden' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0, flexWrap: 'wrap' as const }}>
                        ...
                        <button onClick={() => { setEditId(isEditing ? null : l.id); setEditPrice('') }} style={{ fontSize: 11, color: isEditing ? '#F59E0B' : text3, border: `1px solid ${isEditing ? 'rgba(245,158,11,0.4)' : border}`, padding: '3px 8px', borderRadius: 6, background: 'transparent', cursor: 'pointer' }}>
          ...
          <>
            ...
            <div style={{ background: bg3, border: `1px solid ${border}`, borderRadius: 14, padding: '1.5rem' }}>
              <AddListingForm
                ...
                dark={dark}
                onSaved={() => { loadListings(partner.id); setTab('listings') }}
              />
            </div>
          </>
        )}
          ...
          <>
            ...
            <div style={{ background: bg3, border: `1px solid ${border}`, borderRadius: 14, padding: '1.5rem' }}>
              ...
              <div style={{ background: dark ? 'rgba(91,46,255,0.07)' : 'rgba(91,46,255,0.04)', border: '1px solid rgba(91,46,255,0.15)', borderRadius: 10, padding: '0.875rem 1rem', fontSize: 13, color: text2, marginBottom: 12 }}>
                Want a verified badge? Email <strong>partners@manopintel.com</strong> with your CAC registration. Verification takes 24–48 hours.
              </div>
